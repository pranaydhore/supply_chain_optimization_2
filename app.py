...
# ========================
# CONFIGURATION (FIXED PATHS FOR DEPLOYMENT)
# ========================
...
# ========================
# MAIN TABS (INCLUDES KPI MATRIX TAB)
# ========================
    ...
    if df.empty:
        ...
        if {"efficiency_label", "predicted_probability"}.issubset(df.columns):
            ...
            if not d_roc.empty:
                ...
                roc_auc = auc(fpr, tpr)
                fig_roc = go.Figure()
                fig_roc.add_trace(go.Scatter(x=fpr, y=tpr, mode="lines", line=dict(width=3, color="#1f77b4"), name=f"AUC={roc_auc:.3f}"))
