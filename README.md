...
<title>Supply Chain Optimization Dashboard — Project Overview</title>
<meta name="description" content="A Streamlit + Plotly dashboard for Supply Chain Optimization with ML-based insights, KPI matrix, and model comparison." />
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  body{
    ...
    background:linear-gradient(180deg,var(--bg) 0%, #0b0e1322 100%) fixed, var(--bg);
    color:var(--text);
    line-height:1.6;
  }
  ...
  header.hero{
    ...
    padding:48px 28px;
    background:
      radial-gradient(1400px 400px at 0% -20%, rgba(124,245,255,.15), transparent 60%),
      radial-gradient(1400px 400px at 100% -10%, rgba(91,140,255,.20), transparent 60%),
      linear-gradient(180deg,var(--panel),var(--panel-2));
    box-shadow: var(--shadow);
    overflow:hidden;
  }
  .pill{display:inline-flex;gap:8px;align-items:center;background:rgba(124,245,255,.12);
    border:1px solid rgba(124,245,255,.25); color:var(--brand-2); padding:6px 10px; border-radius:999px; font-size:12px; font-weight:600}
  h1{margin:.2rem 0 0;font-size: clamp(28px, 4vw, 44px); line-height:1.15}
  .sub{color:var(--muted); max-width:68ch; font-size:15.5px}
  .cta{display:flex; gap:12px; margin-top:22px; flex-wrap:wrap}
  .btn{
    background: linear-gradient(180deg,var(--brand),#355fc7);
    border:1px solid rgba(255,255,255,.1);
    color:#fff; padding:12px 16px; border-radius:12px; font-weight:700; box-shadow: var(--shadow);
  }
  .btn.sec{background:transparent; border:1px solid var(--border); color:var(--text)}
  .meta{display:flex; gap:14px; margin-top:14px; color:var(--muted); font-size:13px}
  .badge{padding:2px 8px;border:1px solid var(--border); border-radius:999px}
  .grid{display:grid; gap:18px}
  @media(min-width:900px){ .grid.cols-3{grid-template-columns:repeat(3,1fr)} .grid.cols-2{grid-template-columns:1.1fr .9fr} }
  section.card{
  ...
  .kpi{background:rgba(124,245,255,.06); border:1px solid var(--border); padding:14px 12px; border-radius:14px; text-align:center}
  .kpi .lab{font-size:12px; color:var(--muted)}
  .kpi .val{font-size:18px; font-weight:800}
  pre, code{font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace}
  pre{
    background: var(--code-bg); color:#f1f5f9; border:1px solid var(--code-border);
    padding:16px;border-radius:12px; overflow:auto; box-shadow: inset 0 0 0 1px rgba(255,255,255,.02);
  }
  .toc{position:sticky; top:18px}
  .toc a{display:block; padding:8px 10px; border-radius:8px; color:var(--muted); border:1px dashed transparent}
  ...
  .list-check li:before{content:"▹"; color:var(--accent); position:absolute; left:0}
  footer{color:var(--muted); text-align:center; margin:28px 0}
  .theme-toggle{position:absolute; top:18px; right:18px; display:flex; align-items:center; gap:8px}
  .switch{position:relative; width:50px; height:28px; background:var(--panel-2); border:1px solid var(--border); border-radius:999px; cursor:pointer}
  .knob{position:absolute; top:50%; transform:translateY(-50%); left:3px; width:22px; height:22px; border-radius:50%; background:linear-gradient(180deg,#fff,#cbd5e1); transition:all .2s}
  .light .knob{left:25px}
  ...
  .table th{background:rgba(124,245,255,.08); text-align:left}
  .callout{border-left:4px solid var(--brand); background:rgba(91,140,255,.08); padding:12px 14px; border-radius:8px; margin:12px 0}
</style>
</head>
<body>
  <div class="container">
